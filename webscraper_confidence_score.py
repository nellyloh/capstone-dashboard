...
def search_articles_on_individual(individual_dict, no_of_articles=10, additional_keywords=None):
    '''
    individual dict: dictionary of variables pulled directly from excel
    '''
    ## Adding keywords
    keywords = ['crimes', 'sentenced']
    if additional_keywords:
        for each_keyword in additional_keywords:
            keywords.append(each_keyword)

    def generate_link(person_dict, keywords, attributes_used = ['name']):
    ...
    def generate_link_non_news(person_dict, keywords, attributes_used = ['name']):
        link_start = "https://www.google.com/search?q="
    ...
    search_links = generate_link(individual_dict, keywords=keywords)
        ...
        with requests.Session() as c:
            ...
            for item in soup.find_all('div', attrs = {'class': "ZINbbc xpd O9g5cc uUPGi"}):
                ...
                if link not in unique_links_checker and item:
                    ...
                    current_dict['confidence_score'] = round(conf_score,2)*100
    ...
    if len(output) <= 1 or count_unsatisfactory/len(output) >= 0.7:
        ...
        search_links = generate_link_non_news(individual_dict, keywords=keywords)
